...
** Powerline
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package powerline
       ...
       (defmacro ds/powerline-arrow-tag-left (FACE FUNC &rest ARGS)
         `(ds/powerline-widget arrow ,FACE ,FUNC left nil ,@ARGS))

       (defmacro ds/powerline-left (SEP &rest PARTS)
         (let* ((first-face (car (car PARTS)))
                (start-sep (macroexpand `(ds/powerline-sep ,SEP left 'mode-line ,first-face)))
                (section `(,start-sep))
                (last-face 'mode-line))
           (dolist (part PARTS)
             (let ((face (car part)))
               (if (> (length section) 1) ; add separator only if this is not the first thing
                   (setq section `(,@section ,(macroexpand `(ds/powerline-sep ,SEP left ,last-face ,face)))))
               (dolist (thing (cdr part))
                 (when thing
                   (let* ((hastest (plist-member thing ':cond))
                     ...
                     (if prefix (setq section `(,@section ,(if hastest `(if ,test (powerline-raw ,prefix ,face)) `(powerline-raw ,prefix ,face)))))
                     (setq section `(,@section ,(if hastest `(if ,test (,cmd ,@args ,face)) `(,cmd ,@args ,face))))
                     (if suffix (setq section `(,@section ,(if hastest `(if ,test (powerline-raw ,suffix ,face)) `(powerline-raw ,suffix ,face))))))))
               (setq section `(,@section (powerline-raw " " ,face)))
               (setq last-face face)))
           (setq section `(,@section ,(macroexpand `(ds/powerline-sep ,SEP left ,last-face 'mode-line))))
           `(list ,@section)))
                       ...
                       '("%e"
                         (:eval
                          (let* ((active (powerline-selected-window-active))
                                 ...
                                 (tramp-ssh-face (if active 'powerline-flycheck-info 'powerline-inactive1))
                                 (tramp-sudo-face (if active 'powerline-flycheck-error 'powerline-inactive1))
                                 (tramp-su-face (if active 'powerline-flycheck-error 'powerline-inactive1))
                                 ...
                                 (lhs (append
                                       ...
                                       (when tramp-ssh (ds/powerline-arrow-tag-left tramp-ssh-face powerline-raw " SSH "))
                                       (when tramp-sudo (ds/powerline-arrow-tag-left tramp-sudo-face powerline-raw " SUDO "))
                                       (when tramp-su (ds/powerline-arrow-tag-left tramp-su-face powerline-raw " SU "))
                                       (ds/powerline-left arrow
                                                          (flycheck-face
                                                          ...
                                                          (mode-line
                                                           (:cond (or is-large is-medium) :cmd powerline-vc)))))
                                 (rhs (append
                                       (list (if is-large
...
** Notifications
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       ;; (remove-function (symbol-function 'eosd-dbus-notify) #'ds/exwm-notification-autopop))
   #+END_SRC
