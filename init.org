...
*** Popup Window Width setter
    ...
    #+BEGIN_SRC emacs-lisp
      (defmacro ds/set-popup-width (TARGET-WIDTH)
        "Return a lambda that uses TARGET-WIDTH to set the width of a popup buffer."
        `(lambda (win)
           (if (< (window-width win) ,TARGET-WIDTH)
               (ds/set-window-column-width ,TARGET-WIDTH win))))
    #+END_SRC
    ...
    #+BEGIN_SRC emacs-lisp
      (add-to-list 'display-buffer-alist
                   `(,(rx bos "*compilation*" eos)
                     ...
                     (window-width   . (ds/set-popup-width 106))))
    #+END_SRC
...
** Popup Telegram
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       (add-to-list 'display-buffer-alist
                    `(,(rx bos "TelegramDesktop" eos)
                      (display-buffer-reuse-window
                       display-buffer-in-side-window)
                      (side            . right)
                      (reusable-frames . nil)
                      (inhibit-switch-frame . t)
                      (window-width   . (ds/set-popup-width 135))))


       (exwm-input-set-key (kbd "s-?") #'ds/exwm-popup-telegram)
...
** Popup Mattermost
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       (exwm-input-set-key (kbd "s->") #'ds/exwm-popup-mattermost)
       (push ?\s-> exwm-input-prefix-keys))
   #+END_SRC
...
** Notifications
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       (eosd-start)
       :config
       ;; show notifications in sside window
       (add-to-list 'display-buffer-alist
