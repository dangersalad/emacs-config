  ...
  #+BEGIN_SRC emacs-lisp
    (use-package exwm
      ...
      (with-eval-after-load 'magit
        (defun ds/exwm-fix-magit-workspace-switch ()
          (define-key magit-status-mode-map (kbd "<s-tab>") #'ds/exwm-workspace-toggle))
        (add-hook 'magit-status-mode-hook #'ds/exwm-fix-magit-workspace-switch))
      ...
      (defun ds/exwm-keyrules-termite ()
        (when (and exwm-class-name
          ...
          (setq-local exwm-input-prefix-keys '(?\C-x ?\s-d ?\C-\s-d ?\s-p s-tab ?\C-\s-n))))
