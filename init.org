     ...
     (use-package powerline
       ...
       (zenburn-with-color-variables
         ...
         (defface powerline-active2
           `((t (:background ,zenburn-bg+3 :inherit mode-line)))
           "Powerline face 2."
           :group 'powerline)
         (defface powerline-inactive1
           `((t (:foreground ,zenburn-bg+3 :italic :inherit mode-line-inactive)))
           "Powerline face 1."
           :group 'powerline)
                         ...
                         (:eval
                          (let* ((active (powerline-selected-window-active))
                                 ...
                                 (lhs (list (powerline-raw " " mode-line)
                                            (unless is-exwm-window (powerline-raw "%*" mode-line 'l))
                                            (unless is-exwm-window
                                              (when (and powerline-display-buffer-size
                                                         is-large)
                                                (powerline-buffer-size mode-line 'l)))
                                            (unless is-exwm-window
                                              (when (and powerline-display-mule-info
                                                         is-large)
                                                (powerline-raw mode-line-mule-info mode-line 'l)))
                                            (powerline-raw " " mode-line)
                                            (when tramp-ssh (funcall separator-left mode-line 'powerline-flycheck-info powerline-height))
                                            (when tramp-ssh (powerline-raw "SSH" 'powerline-flycheck-info))
                                            (when tramp-ssh (funcall separator-left 'powerline-flycheck-info mode-line powerline-height))
                                            (when tramp-sudo (funcall separator-left mode-line 'powerline-flycheck-error powerline-height))
                                            (when tramp-sudo (powerline-raw "SUDO" 'powerline-flycheck-error))
                                            (when tramp-sudo (funcall separator-left 'powerline-flycheck-error mode-line powerline-height))
                                            (when tramp-su (funcall separator-left mode-line 'powerline-flycheck-error powerline-height))
                                            (when tramp-su (powerline-raw "SU" 'powerline-flycheck-error))
                                            (when tramp-su (funcall separator-left 'powerline-flycheck-error mode-line powerline-height))
                                            (funcall separator-left mode-line flycheck-face powerline-height)
                                            (powerline-buffer-id flycheck-face 'l)
                                            (when (and (boundp 'which-func-mode) which-func-mode is-large)
                                              (powerline-raw which-func-format flycheck-face 'l))
                                            (powerline-raw " " flycheck-face)
                                            (funcall separator-left flycheck-face face1 powerline-height)
                                            (when (and (boundp 'erc-modified-channels-object) is-large)
                                              (powerline-raw erc-modified-channels-object face1 'l))
                                            (powerline-major-mode face1 'l)
                                            (when is-large (powerline-process face1))
                                            (when (or is-large is-medium) (powerline-minor-modes face1 'l))
                                            (when is-large (powerline-narrow face1 'l))
                                            (powerline-raw " " face1)
                                            (funcall separator-left face1 mode-line powerline-height)
                                            (when (or is-large is-medium) (powerline-vc mode-line 'r))))
                                 (rhs (list (if is-large
                                                (funcall separator-right mode-line face1 powerline-height)
                                              (funcall separator-right mode-line flycheck-face powerline-height))
                                            (when is-large (powerline-raw global-mode-string face1 'r))
                                            (if is-large
                                                (funcall separator-right face1 flycheck-face powerline-height)
                                              (funcall separator-right mode-line flycheck-face powerline-height))
                                            (unless is-exwm-window (powerline-raw "%4l" flycheck-face 'l))
                                            (unless is-exwm-window (powerline-raw "%4c" flycheck-face 'r))
                                            (funcall separator-right flycheck-face mode-line powerline-height)
                                            (powerline-raw " " mode-line)
                                            (unless is-exwm-window (when is-large (powerline-raw "%6p" mode-line 'r)))
                                            (powerline-raw " " mode-line))))
       ...
       ;;   (ds/powerline-theme))
       (zenburn-with-color-variables
         (set-face-attribute 'powerline-active1 nil :background zenburn-bg+2)
         (set-face-attribute 'powerline-active2 nil :background zenburn-bg+3))
...
** Windows
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       :config
       (exwm-input-set-key (kbd "C-s-k") #'ds/exwm-window-grow-above)
       (push ?\C-\s-k exwm-input-prefix-keys)
       (exwm-input-set-key (kbd "C-M-s-k") #'ds/exwm-window-shrink-above)
       (push ?\C-\M-\s-k exwm-input-prefix-keys)

       (exwm-input-set-key (kbd "C-s-j") #'ds/exwm-window-grow-below)
       (push ?\C-\s-j exwm-input-prefix-keys)
       (exwm-input-set-key (kbd "C-M-s-j") #'ds/exwm-window-shrink-below)
       (push ?\C-\M-\s-j exwm-input-prefix-keys)

       (exwm-input-set-key (kbd "C-s-h") #'ds/exwm-window-grow-left)
       (push ?\C-\s-h exwm-input-prefix-keys)
       (exwm-input-set-key (kbd "C-M-s-h") #'ds/exwm-window-shrink-left)
       (push ?\C-\M-\s-h exwm-input-prefix-keys)

       (exwm-input-set-key (kbd "C-s-l") #'ds/exwm-window-grow-right)
       (push ?\C-\s-l exwm-input-prefix-keys)
       (exwm-input-set-key (kbd "C-M-s-l") #'ds/exwm-window-shrink-right)
        ...
        (defun ds/exwm-keyrules-termite ()
          (when (and exwm-class-name
            ...
            (setq-local exwm-input-prefix-keys
                        '(?\C-x ?\s-d ?\C-\s-d ?\s-p s-tab ?\C-\s-n
                                ?\C-n ?\s-o ?\C-\s-o ?\M-x ?\M-:
                                ?\C-\s-q ?\s-h ?\s-j ?\s-k ?\s-l))))
...
** Notifications
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package exwm
       ...
       (add-function :after (symbol-function 'eosd-dbus-notify) #'ds/exwm-notification-autopop))
   #+END_SRC
