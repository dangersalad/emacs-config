   ...
   #+BEGIN_SRC emacs-lisp
     (use-package flycheck
       ...
       (defun ds/flycheck-close-unused-list (&rest _)
         (if (or (and (not flycheck-enabled-checkers)
    ...
    (use-package exwm
      ...
      (exwm-input-set-key (kbd "s-C-l")
                          (lambda ()
                            ...
                            (start-process "" nil
                                           (concat
                                            (concat user-emacs-directory "bin/lock")
                                            " "
                                            (concat user-emacs-directory "lib/lock.png")))))

      ;; popup eshell
      (defun ds/exwm-popup-shell ()
        (interactive)
        (let ((win (selected-window))
              (current-popup (get-buffer-window ds/exwm-popup-shell-buffer))
              (popup-buf (get-buffer ds/exwm-popup-shell-buffer))
              (eshell-buffer-name ds/exwm-popup-shell-buffer))
          (if (equal win current-popup)
              (if (= (length (window-list)) 1)
                  (save-selected-window
                    (bury-buffer ds/exwm-popup-shell-buffer)
            ...
            (if current-popup
              ...
              (if popup-buf
                ...
                (eshell ds/exwm-popup-shell-buffer))))))
