     ...
     (use-package powerline
               ...
               (dolist (thing (cdr part))
                 (when (and thing (listp thing))
                   (let* ((hastest (plist-member thing ':cond))
                     ...
                     (when (or (not hastest) (eval test))
                       (if prefix (setq face-section `(,@face-section ,(if hastest `(if ,test (powerline-raw ,prefix ,face)) `(powerline-raw ,prefix ,face)))))
                       (setq face-section `(,@face-section ,(if hastest `(if ,test (,cmd ,@args ,face)) `(,cmd ,@args ,face))))
                       (if suffix (setq face-section `(,@face-section ,(if hastest `(if ,test (powerline-raw ,suffix ,face)) `(powerline-raw ,suffix ,face)))))))))
               (when (> (length face-section) 1)
       ...
       (zenburn-with-color-variables
         ...
         (defface ds/powerline-gray
           `((t (:foreground ,zenburn-fg :background ,zenburn-bg+2 :inherit mode-line)))
           "Powerline Gray."
           :group 'powerline)
         (defface ds/powerline-light-gray
           `((t (:foreground ,zenburn-bg-2 :background ,zenburn-bg+3 :inherit mode-line)))
                       ...
                       '("%e"
                         (:eval
                          (let* (;; size info
                                 ...
                                 (tramp-face (if active 'ds/powerline-blue 'ds/powerline-inactive))
                                 (tramp-ssh-face (if active 'ds/powerline-green 'ds/powerline-inactive))
                                 ...
                                 ;; left side
                                 (lhs (append
                                       (ds/powerline-section arrow left
                                                             (mode-line
                                                             ...
                                                             (tramp-face
                                                              (:cond is-tramp :args ("Tramp")))
                                                             (tramp-ssh-face
                                                              (:cond tramp-ssh :args ("SSH")))
                                                             (tramp-su-face
                                                              (:cond tramp-su :args ("SU")))
                                                             (tramp-sudo-face
                                                              (:cond tramp-sudo :args ("SUDO")))
                                                             (flycheck-face
                                 ...
                                 ;; right side
                                 (rhs (ds/powerline-section arrow right
                                                            (inner-face
